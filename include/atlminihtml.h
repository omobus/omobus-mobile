...
public:
         ...
         case CMD_DRAW:
            {
               ...
               if( m_bWordWrap ) {
                  ...
                  while( nCopied < action.cchMax ) {
                     ...
                     dc.DrawText(pstr, nLen, &rcPos, DT_WORDBREAK | DT_TOP | DT_NOPREFIX | uStyle);
                     pstr += nLen;
                     nCopied += nLen;
                  ...
                  dc.DrawText(pstrText, action.cchMax, &rcPos, DT_WORDBREAK | DT_TOP | DT_NOPREFIX | uStyle);
                  rcPos.left += sizeText.cx;
               }
            }
            break;
   ...
   void _ParseTag(CDCHandle dc, LPCTSTR& pstrText, CONTEXT& c, FONT& f, int& iHeight)
   {
      ...
      if( szTag[0] != _T('/') ) 
      {
         ...
         else if( ::lstrcmp(szTag, _T("BR")) == 0 || ::lstrcmp(szTag, _T("BR/")) == 0 ) {
...
public:
   ...
   void DoPaint(CDCHandle dc)
   {
      ...
      CString pstrText;
      GetWindowText(pstrText);

      COLORREF clrText = m_clrText == CLR_INVALID ? ::GetSysColor(COLOR_BTNTEXT) : m_clrText;
      COLORREF clrBack = m_clrBack == CLR_INVALID ? ::GetSysColor(COLOR_WINDOW) : m_clrBack;
      HFONT hFont = GetFont();

      m_minihtml.Paint(dc, rcClient, pstrText, clrText, clrBack, hFont);
   }
};
