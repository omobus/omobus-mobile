...
#define DDX_CUR(nID, var, prec) \
		if(nCtlID == (UINT)-1 || nCtlID == nID) \
		{ \
			if(!DDX_Cur(nID, var, bSaveAndValidate, prec)) \
				return FALSE; \
		}
...
// Currency exchange
  static double GetDlgItemDouble(HWND hWnd, UINT id) {
    wchar_t buf[255] = _T(""), *xxx = NULL;
    GetDlgItemText(hWnd, id, buf, 254);
    buf[254] = 0;
    return _tcstod(buf, &xxx);
  }

  BOOL DDX_Cur(UINT nID, double& nVal, BOOL bSave, INT cur_prec)
	{
		T* pT = static_cast<T*>(this);
		BOOL bSuccess = TRUE;

		if(bSave)
			nVal = GetDlgItemDouble(pT->m_hWnd, nID);
		else
      pT->SetDlgItemText(nID, ftows(nVal, cur_prec).c_str());
