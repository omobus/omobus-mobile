...
#define WTL_STD_SHIDIF   SHIDIF_CANCELBUTTON | SHIDIF_SIPDOWN | SHIDIF_SIZEDLGFULLSCREEN
...
// Overrideables
  ...
  const wchar_t *GetSubTitle() {
    ...
    const wchar_t *sSubTitle = pT->GetSubTitle();

		// Display title text
		RECT rTitle = { 0, 0, nWidth, nTitleHeight };
		dc.FillRect(&rTitle, COLOR_WINDOW);
	#ifdef _WTL_CE_DRA
		rTitle.left = DRA::SCALEX(/*8*/4);
		rTitle.right -= DRA::SCALEX(/*8*/4);
	#else // !_WTL_CE_DRA
		rTitle.left = nTitleHeight / /*3*/6; // 8 == 24 / 3
		rTitle.right = nTitleHeight / /*3*/6; // 8 == 24 / 3
	#endif // !_WTL_CE_DRA
		dc.DrawText(sTitle, nLen, &rTitle, DT_VCENTER | DT_SINGLELINE);
    if( sSubTitle != NULL ) 
      dc.DrawText(sSubTitle, -1, &rTitle, DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    dc.SelectFont(fontOld);

    CPen dashPen, oldPen;
    dashPen.CreatePen(PS_DASH, 1, RGB(165, 162, 165));
    oldPen = dc.SelectPen(dashPen);
    dc.MoveTo(0, nTitleHeight); dc.LineTo(nWidth, nTitleHeight);
    dc.SelectPen(oldPen);
