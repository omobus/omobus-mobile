...
// Attributes
	...
	bool GetIdealSize(int& cx, int& cy) const
	{
		...
		if(IsUsingTags())
		{
			...
			dc.DrawText(lpstrLink, cchLink, &rcLink, DT_LEFT | DT_WORDBREAK | DT_NOPREFIX | DT_CALCRECT);
		...
		else
		{
			...
			if (dwStyle & SS_CENTER)
				...
				nDrawStyle = DT_RIGHT;
			dc.DrawText(lpstrText, -1, &rcAll, nDrawStyle | DT_WORDBREAK | DT_NOPREFIX |DT_CALCRECT);
			if(m_hFont != NULL)
...
// Implementation
	...
	bool CalcLabelRect()
	{
		...
		else
		{
			...
			if (dwStyle & SS_CENTER)
				...
				nDrawStyle = DT_RIGHT;
			dc.DrawText(lpstrText, -1, &m_rcLink, nDrawStyle | DT_WORDBREAK | DT_NOPREFIX | DT_CALCRECT);
			if(m_hFont != NULL)
				dc.SelectFont(hOldFont);
			if (dwStyle & SS_CENTER)
			{
	...
	void DoPaint(CDCHandle dc)
	{
		if(IsUsingTags())
		{
			...
			dc.DrawText(lpstrLink, cchLink, &m_rcLink, DT_LEFT | DT_WORDBREAK | DT_NOPREFIX);
		...
		else
		{
			...
			dc.DrawText(lpstrText, -1, &m_rcLink, nDrawStyle | DT_WORDBREAK | DT_NOPREFIX);
